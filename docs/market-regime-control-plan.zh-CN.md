...
- `macro_risk_governor`
  负责宏观降杠杆。它看价格趋势、实现波动、VIX 和信用 ETF 相对压力，输出 `risk_reduced` 或 `risk_off`。
  HY OAS、金融压力指数、Fear & Greed、put/call、safe-haven demand、VVIX、SKEW、MOVE、收益率曲线、美元压力、市场宽度、AAII/NAAIM 和五角大楼比萨指数默认只作为 watch-only 证据，用于通知和回测观察。
...
- Global ETF、Russell 1000、Mega Cap 类轮动策略
  可以先生成统一插件 artifact，用于通知和证据归档；但策略级 `position_control_allowed` 在专门的 25-30 年长周期验证包归档前保持 false。后续若推广，候选规则仍是：`risk_reduced` 做 50% 风险预算缩放，`risk_off` 归零风险资产预算。
- DCA 或收入型低频策略
...
- 轮动策略：在 25-30 年长周期验证包归档前保持 notification-only。后续推广候选规则是 `risk_reduced` 做 50% risk scaling，`risk_off` 归零风险资产预算。
