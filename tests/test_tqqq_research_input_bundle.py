...
def test_serializer_canonicalizes_equivalent_records_and_rejects_incomplete_last_qqq() -> None:
    ...
    next(record for record in reversed(records) if record["symbol"] == "QQQ")["volume"] = None
    with pytest.raises(bundle.BundleError) as error:
...
def test_provider_normalized_timestamp_is_canonicalized_to_exact_date() -> None:
    records = _records()
    for record in records:
        record["as_of"] = pd.Timestamp(record["as_of"])

    raw = bundle.serialize_price_rows(records, end_exclusive="2026-08-01")

    assert all(len(row["as_of"]) == 10 for row in bundle.parse_price_rows(raw))
    for value in (pd.Timestamp("2026-07-21T00:00:00+00:00"), pd.Timestamp("2026-07-21T00:00:01")):
        invalid = _records()
        invalid[0]["as_of"] = value
        with pytest.raises(bundle.BundleError) as error:
            bundle.serialize_price_rows(invalid, end_exclusive="2026-08-01")
        assert error.value.code == bundle.PROVIDER_DATA_INVALID


def test_bundle_requests_spy_for_existing_market_symbol_default(
    tmp_path: Path, source_gate: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    expected_requested = ["QQQ", "SPY", "TQQQ", "^VIX", "^VIX3M", "HYG", "IEF", "LQD", "XLF", "KRE", "TLT"]
    assert list(bundle.REQUESTED_SYMBOLS) == expected_requested

    destination, calls = _build(tmp_path, source_gate, monkeypatch)
    manifest = json.loads((destination / "manifest.json").read_bytes())
    raw_symbols = {row["symbol"] for row in bundle.parse_price_rows((destination / "prices.csv").read_bytes())}

    assert calls == 1
    assert raw_symbols == set(expected_requested)
    assert manifest["provider"]["requested_symbols"] == expected_requested
    assert manifest["prices"]["symbols"] == sorted(expected_requested)
    assert b"market_symbol" not in bundle.CONFIG_BYTES
    from quant_strategy_plugins.crisis_context_research import DEFAULT_MARKET_SYMBOL

    assert DEFAULT_MARKET_SYMBOL == "SPY"
